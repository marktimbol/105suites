 ...
 */
class GAPGoogle_Logger_File extends GAPGoogle_Logger_Abstract
{
  /**
  ...
  public function __construct(GAPGoogle_Client $client)
  {
    ...
    $file = $client->getClassConfig('GAPGoogle_Logger_File', 'file');
    if (!is_string($file) && !is_resource($file)) {
      throw new GAPGoogle_Logger_Exception(
          'File logger requires a filename or a valid file pointer'
      );
    }

    $mode = $client->getClassConfig('GAPGoogle_Logger_File', 'mode');
    if (!$mode) {
      $this->mode = $mode;
    }

    $this->lock = (bool) $client->getClassConfig('GAPGoogle_Logger_File', 'lock');
    $this->file = $file;
  }
  ...
  protected function write($message)
  {
    if (is_string($this->file)) {
      ...
      throw new GAPGoogle_Logger_Exception('File pointer is no longer available');
    }
