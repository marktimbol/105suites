...
class FusionSC_Testimonials {
	...
	public function render_parent( $args, $content = '' ) {
		...
		$html = sprintf( '<div %s>%s<ul %s>%s</ul>%s</div>', FusionBuilder::attributes( 'testimonials-shortcode' ), $styles,
	...
	public function attr() {
		...
		$attr = fusion_builder_visibility_atts( self::$parent_args['hide_on_mobile'], array(
			'id'	=> 'Testimonials__slider',
			'class' => 'fusion-testimonials testimonials-flexslider ' . self::$parent_args['design'] . ' fusion-testimonials-' . $this->testimonials_counter,
		) );
	...
	public function testimonials_attr() {
		return array(
			'class' => 'reviews_ testimonials slides',
		);
	}
	...
	private function render_child_clean( $content ) {
		...
		$html = sprintf(
			'<li %s>%s<div class="review__information"><blockquote %s><q %s>%s</q></blockquote>%s</div></li>',
			FusionBuilder::attributes( 'testimonials-shortcode-review' ),
			$thumbnail,
